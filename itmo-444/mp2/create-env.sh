...
echo "First creating backend..."
aws ec2 run-instances --image-id $imgid --count 1 --instance-type t2.micro --key-name $keyname --security-groups $secgrp --iam-instance-profile $iamprofile --user-data file://backend.sh
avilzone=($(aws ec2 run-instances --image-id $imgid --count $count --instance-type t2.micro --key-name $keyname --security-groups $secgrp --iam-instance-profile $iamprofile --user-data file://frontend.sh | grep  AvailabilityZone | awk '{ print $2 }' | tr -d '",[]'))
