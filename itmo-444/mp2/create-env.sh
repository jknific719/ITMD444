...
avilzone=($(aws ec2 run-instances --image-id $imgid --count $count --instance-type t2.micro --key-name $keyname --security-groups $secgrp --iam-instance-profile $iamprofile --user-data file://create-app.sh | grep  AvailabilityZone | awk '{ print $2 }' | tr -d '",[]'))
